    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, dcn=None):
        ...
        if not self.with_dcn or fallback_on_stride:
            ...
            if not self.with_modulated_dcn:
                from modules.dcn import DeformConv
                conv_op = DeformConv
                offset_channels = 18
            else:
                from modules.dcn import ModulatedDeformConv
...
resnet_spec = {18: (BasicBlock, [2, 2, 2, 2], [64, 128, 256, 512]),
...
def get_resnet_backbone(num_layers=18, pretrained=True, **kwargs):
    block_class, layers, out = resnet_spec[num_layers]
    model = ResNet(block_class, layers, **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet%d' % num_layers]), strict=False)
    return model, out
...
if __name__ == '__main__':
    ...
    model, out = get_resnet_backbone(18)
    net = model.cuda()
    dummy_input = torch.ones(1, 3, 128, 128, device='cpu')
